...
function useLocation() {
  ...
  const getMyLocation = async () => {
    ...
    const onError = error => {
      setIsLocationLoading(false);
      if (error.PERMISSION_DENIED == 1) alert(Strings['Please turn on your location']);
        ...
        if (granted === PermissionsAndroid.RESULTS.GRANTED) {
          const grantedBackground = await PermissionsAndroid.request(
            PermissionsAndroid.PERMISSIONS.ACCESS_BACKGROUND_LOCATION,
            {
              title: 'App',
              message:
                'App wants to access your device location to track your live location updates, even when the app is closed or not in use,',
            },
          );
          response = true;
        } else {
  ...
  return [location, getMyLocation, isLocationLoading, reverseGeoCoding];
}
